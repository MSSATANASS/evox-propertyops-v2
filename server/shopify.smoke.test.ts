 ...
 *
 * Goal: prove the Storefront API is reachable. When the catalog has products,
 * it also verifies that at least one is usable by a storefront. A new store is
 * allowed to return an intentionally empty catalog until an operator defines
 * real offerings and prices.
 *
 * Behavior:
...
describe.skipIf(!configured)("shopify smoke (live)", () => {
  it(
    "returns an accessible catalog and validates a usable product when populated",
    { timeout: 30_000 },
    async () => {
